...
function expectSync(v: Vector | Promise<Vector>, dim: number) {
...
export function v2(
	...
	...rest: readonly (number | [number, number] | Vector2)[]
): Vector2 {
	const array = isTemplateStringsArray(v)
		? expectSync(vector(v, ...rest), 2)
...
export function v3(
	...
	...rest: readonly (number | [number, number, number] | Vector3)[]
): Vector3 {
	const array = isTemplateStringsArray(v)
		? expectSync(vector(v, ...rest), 3)
