...
export type ComputedMethod<F> = F extends (...args: infer Args) => infer Result
	? (...args: ComputedArgs<Args>) => Computed<Awaited<Result>>
	: never

export type ComputedFunction<F> = F extends (...args: infer Args) => infer Result
	? (...args: ComputedArgs<Args>) => Computed<Awaited<Result>>
	: never

export type ComputedMembers<T> = {
	[K in keyof T as T[K] extends UnknownFunction ? K : never]: ComputedMethod<
		Extract<T[K], UnknownFunction>
	>
}
...
export const computedClassFactorySymbol = Symbol('computedClassFactory')

export interface InternalComputedClass<Base extends Constructor = Constructor>
	extends ComputedClass<Base> {
